...
Theorem wp_goal :
  ...
  (forall (i3:Numbers.BinNums.Z), (i3 < i1)%Z -> (i <= i3)%Z ->
   ((t3 (shift (t2 (shift a i3)) i2)) = (t1 (shift (t (shift a1 i3)) i2)))) ->
  ((L_sum t2 t3 a i i1 i2) = (L_sum t t1 a1 i i1 i2)).
(* Why3 intros t t1 t2 t3 a a1 i i1 i2 h1 h2 h3. *)
Proof.
intros M M1 M2 M3 t1 t2 i j c H1 H2 H.
...
- generalize dependent H.
  ...
  (fun j => 
  (forall i3 : int,
  (i3 < j)%Z -> (i <= i3)%Z -> M3 (shift (M2 (shift t1 i3)) c) = M1 (shift (M (shift t2 i3)) c)) ->
   L_sum M2 M3 t1 i j c = L_sum M M1 t2 i j c) i).
  intros.
