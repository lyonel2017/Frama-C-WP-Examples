...
(* Why3 goal *)
Theorem wp_goal :
  forall (t:addr -> addr) (t1:addr -> Numbers.BinNums.Z) (a:addr)
    (i:Numbers.BinNums.Z) (i1:Numbers.BinNums.Z) (i2:Numbers.BinNums.Z)
    (i3:Numbers.BinNums.Z),
  (i1 <= i2)%Z -> (i <= i1)%Z -> (0%Z <= i)%Z -> is_sint32_chunk t1 ->
  (((L_sum t t1 a i i1 i3) + (L_sum t t1 a i1 i2 i3))%Z =
   (L_sum t t1 a i i2 i3)).
Proof.
intros.
apply Zlt_lower_bound_ind with
(P:= fun k => 
(L_sum t t1 a i i1 i3 + L_sum t t1 a i1 k i3)%Z = L_sum t t1 a i k i3)
(z := i1) ;[| omega].
intros.
assert (H5:(i1 = x \/ i1 < x)%Z) by omega.
destruct H5.
- rewrite H5.
  rewrite (Q_sum1 _ _ _ x x).
